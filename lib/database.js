  ...
  for (const item of items) {
    ...
    if (existing) {
      ...
      Object.assign(existing, {
        ...
        content: item.content || existing.content, // AI üretilen tam içerik
        source: item.source || existing.source,
        category: item.category || existing.category,
        publishedAt: item.publishedAt || existing.publishedAt,
        aiSummary: item.aiSummary || existing.aiSummary,
        aiGenerated: item.aiGenerated !== undefined ? item.aiGenerated : existing.aiGenerated,
        importance: item.importance !== undefined ? item.importance : existing.importance,
        tags: item.tags || existing.tags || [],
        researchTopic: item.researchTopic || existing.researchTopic,
      ...
      newsMap.set(item.link, {
        ...
        content: item.content || null, // AI üretilen tam içerik
        source: item.source || 'Bilinmeyen',
        category: item.category || null,
        publishedAt: item.publishedAt || new Date().toISOString(),
        aiSummary: item.aiSummary || null,
        aiGenerated: item.aiGenerated || false, // AI üretilen mi?
        importance: item.importance || null, // Önem skoru
        tags: item.tags || [], // Etiketler
        researchTopic: item.researchTopic || null, // Araştırma konusu
