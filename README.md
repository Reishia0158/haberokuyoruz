...
- ✨ **40+ haber kaynağı** - TRT, Habertürk, Sözcü, NTV ve daha fazlasından RSS çekme
...
- 💾 **Otomatik veritabanı** - JSON tabanlı, RSS kesilse bile içerikler korunur
...
2. Anahtarı `.env` dosyasına ya da kabuğunda ortam değişkeni olarak tanımla:
   ...
   GEMINI_MODEL="gemini-1.5-flash-001"      # opsiyonel (varsayılan)
   GEMINI_SUMMARY_MAX_ITEMS=20              # opsiyonel, özetlenecek haber sayısı
   AI_ANALYSIS_LIMIT=30                     # opsiyonel, AI analiz edilecek haber sayısı
...
- `lib/ai-news-manager.js`: AI destekli haber analiz ve yönetim sistemi.
- `lib/database.js`: JSON tabanlı veritabanı sistemi.
- `public/`: HTML, CSS ve tarayıcı tarafı JS.

## Özelleştirme

- Yeni kaynak eklemek için `server.js` içindeki `RSS_SOURCES` listesine `{ name, url, category }` objesi ekle.
...
- `AI_HABER_SISTEMI.md` - AI sistem detayları
